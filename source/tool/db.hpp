...
public:
     ...
     // 注册时新增用户
     bool insert(Json::Value &user) //["id","用户名"，"密码"，"天梯分数"，""]
     {
#define INSERT_USER "insert user values(null, '%s', '%s', 1000, 0, 0);"
          ...
          char sql[4096] = {0};
          // int sprintf(char *str, const char *format, ...); 成功返回写入字符串的字符数（不包括结尾的 '\0'）
     ...
     bool login(Json::Value &user)
     {
          ...
          // 以用户名和密码共同作为查询过滤条件，查询到数据则表示用户名密码一致
#define LOGIN_USER "select id, score, total_count, win_count from user where username='%s' and password='%s';"
          ...
          MYSQL_RES *res = NULL;
          {
               ...
               res = mysql_store_result(_mysql); // 保存本地
               if (res == NULL)
               {
          ...
          if (row_num != 1) // 健壮性判断，其实在数据库中已保证用户名的唯一性
          {
